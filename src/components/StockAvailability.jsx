...
const StockAvailability = ({ stockOpen, setStockOpen }) => {
  ...
  return (
    <div className='bg-white flex justify-between items-center w-[491px] h-[80px] rounded-[15px] shadow-md px-4'>
      <div className={` font-semibold ${stockOpen ? "text-[#00C678]" : "text-[#FF3C26]"}`}>
        <p className=' text-[12px]'>{stockOpen ? stockAvailable.label : "Out Of stock"}</p>
        <div className='flex items-center gap-8'>
          <p className=' text-[18px]'>{stockOpen ? stockAvailable.value : 15}</p>
        </div>
      </div>
